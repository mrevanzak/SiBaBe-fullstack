    ...
    Router::new()
        ...
        .mutation("create", |t| {
            t(|ctx, input: prisma::products::Data| async move {
                let create_product = ctx.db.products().create(input.name, input.description, input.price, input.stock, input.image, vec![]).exec().await.map_err(|e| {
        ...
        .mutation("edit", |t| {
            t(|ctx, input: EditProduct| async move {
                let edit_product = ctx.db.products().update(prisma::products::id::equals(input.id), vec![
